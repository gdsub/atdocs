...
2. 根据 [验证 Cloud API 服务](https://cloud.google.com/vision/docs/common/auth) 指南来为您的工程生成一个新的 Android API 密钥。

3. Add the Cloud Vision Java client library dependencies to your app-level `build.gradle` file:

3. 在您应用的 `build.gradle`  文件中添加 Cloud Vision Java 客户端的库依赖：
...
1.  使用您的云工程的 API 密钥创建一个 `VisionRequestInitializer` 对象。
...
3. 将图像数据转码并传递给 `Image` 实例。然后将这个实例传递给一个 `AnnotateImageRequest` 实例并同时设置 `LABEL_DETECTION` 参数来请求相关特征。
